...
$IngestConfig = Join-Path $PipelineDir "configs\bling_ingest_hub_v1.json"
$GeneratorScript = Join-Path $PipelineDir "06_generate_bling_supabase_import.py"
$ReconcileScript = Join-Path $PipelineDir "09_reconcile_bling_supabase.py"
$BlingSecretsPath = Join-Path $ClearRoot "00_governanca\politicas_de_acesso\legacy_credenciais\bling id.txt"
$StatusDir = Join-Path $PipelineDir "out\status"
$LogDir = Join-Path $PipelineDir "out\logs"
...
Invoke-Step "Finance ingest hub (bling)" {
    ...
    Invoke-Step "Generate Bling -> Supabase migration" {
        Set-Location $ClearRoot
        python $GeneratorScript --from-date $FromDate --run-id ("bling_import_v1_{0}" -f $RunId) --batch-size 400 --company $CompanyTag
    }
...
if (-not $SkipRecon) {
    ...
    Invoke-Step "Reconcile Bling x Supabase (AP/AR)" {
        Set-Location $ClearRoot
        python $ReconcileScript --from-date $FromDate --run-id ("bling_recon_{0}" -f $RunId) --company $CompanyTag --supabase-token-path $SupabaseTokenPath
    }
} else {
